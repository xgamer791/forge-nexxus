...
- `convex/` holds the schema, Convex Auth setup (`auth.ts`), and the conversation, message, user, connection, settings, and workspace functions. `convex.json` marks `ssh2` as an external package so Convex installs it on the deployment rather than bundling its optional native dependencies, and `convex/shapes.ts` holds the validators shared with the Node action module so it never imports the schema. `npx convex dev` pushes them to the deployment named in `.env.local` (`CONVEX_DEPLOYMENT` plus `CONVEX_DEPLOY_KEY`, never committed) and regenerates `convex/_generated/`.
...
- Deploying is a separate step from the Pages deploy: pushing to `main` publishes `docs/`, but new tables and functions only exist once someone with the deploy key runs `npx convex deploy`. A client calling a function the deployment does not have yet fails with `Could not find public function for '<module>:<name>'`.
- `apps` caches the applications found on a server. `remote.scanApps` reads them over the same connection (`~/applications/*` on Cloudways, `/var/www/*` otherwise, preferring a `public_html` doc root), and each scan replaces that server's rows, so an app deleted on the server disappears here. The scan runs when a server connects and once per signed-in session, so the list is what the server holds now rather than what a previous login cached.
- One workspace is active at a time, an app or a repository. `apps.activate` and `connections.setConnected` each stand the other down, and `apps.usedAt` is what puts an app in Recents: being discovered by a scan is not the same as having been chosen.
