...
- **When the route looks right and builds still fail:** `npx convex run probe:chat` makes one small call to the configured chat route and reports what came back — HTTP status, which fields the reply carried, how many characters went to `content` and how many to a reasoning field, `finish_reason`, and the provider's own error. It describes the key (length, stray whitespace from a paste, masked ends) but never returns it, and scrubs the key out of anything the provider echoes. `generate:routing` says where a turn is sent; this says what answers. A reply with empty `content` and a large `reasoningChars` is a reasoning model spending the length budget on thinking: raise `AI_MAX_TOKENS` or choose another model, because retrying buys the same answer.
