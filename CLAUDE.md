...
- **Chat/build:** DeepSeek v4.1 Flash. Convex `AI_MODEL=deepseek-flash`, `AI_BASE_URL=https://api.deepseek.com/v1`, `AI_API_KEY` set. `AI_MODEL_LABEL` is unset so the fallback pretty name is used. Hardcoded fallbacks in `convex/generate.ts` match that route. **Builds:** `AI_BUILD_MODEL` is unset, so a build runs on `deepseek-flash` exactly as chat does. `npx convex run probe:models` lists two ids on this key — `deepseek-flash` and `deepseek-v4-pro` — and pointing builds at the stronger one is `npx convex env set AI_BUILD_MODEL deepseek-v4-pro`, one command with no deploy, and `npx convex env remove AI_BUILD_MODEL` to come back. It was tried and deliberately reverted: Flash is the route this deployment runs. Flash is sent no thinking control at all: it ignores them. One puzzle run nine times gave 1,431–4,609 thinking characters with no field, 2,043–3,026 at `reasoning_effort: high`, and 1,282–2,035 with `enable_thinking: false` — overlapping, and a switch that cannot stop it thinking is not being read. The provider returns 200 for unknown fields and drops them, so only comparing runs tells you anything. `deepseek-v4-pro` does appear to read it (1,409/1,617 at `low` against 4,702/15,856 at `high`), on two runs a side that want re-measuring before anything is built on them. That field was once Gemini's alone, on the grounds that it broke every other provider — measured on this route it does not: asked one puzzle, `deepseek-v4-pro` thought 1,409–1,617 characters at `low` against 4,702–15,856 at `high`, `deepseek-flash` 2,574 against 6,836, and every call answered 200. So it is sent for the models in `EFFORT_MODELS` and on a Gemini host, and nowhere else: an unmeasured provider still gets a plain body. A chat turn stays plain even on those models, because the strategist and the memory note ride that route and neither should pay for a long think. `AI_REASONING_EFFORT` accepts `low` / `medium` / `high`; unset lands on `high`.
...
- **Verify without secrets:** `npx convex run generate:routing` on polished-ram-883. It returns three rows: `chat` on `deepseek-flash` with `reasoningEffort: null` and `maxTokens: 32000`, `build` on the same model with `reasoningEffort: null`, `maxTokens: 96000` and `sameAsChat: true`, both at host `api.deepseek.com`, and `image` on `gemini-3.1-flash-lite-image` with `pinnedToLite: false`. There is no `misrouted` or `refuseReason` field any more — the route guard was removed, so nothing is judged before it is tried. A session that cannot reach the deployment can run `npx vitest run convex/routing.test.ts convex/buildFlow.test.ts` instead, which drives the same settings through the real build pipeline.
