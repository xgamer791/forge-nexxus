...
- **Chat/build:** Gemini 3.8 Flash at high reasoning effort. Convex `AI_MODEL=gemini-3.8-flash`, `AI_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai`, `AI_API_KEY` set to the existing `GEMINI_API_KEY` value. `AI_MODEL_LABEL` is unset so the fallback pretty name is used. `AI_REASONING_EFFORT` is unset so the code default `high` is sent as `reasoning_effort` on every OpenAI-compat chat/build call. Hardcoded fallbacks in `convex/generate.ts` match that route.
- **Images:** Gemini Nano Banana 2 Lite. `GEMINI_API_KEY` (same value as Convex `AI_IMAGE_API_KEY`) in `.env.local` and GitHub secret `GEMINI_API_KEY`. `AI_IMAGE_MODEL=gemini-3.1-flash-lite-image`. Pictures stay on this native image route; they never go through Gemini 3.8 Flash.
- **Verify without secrets:** `npx convex run generate:routing` on polished-ram-883. It returns three rows: `chat` and `build` both on `gemini-3.8-flash` at host `generativelanguage.googleapis.com` with `reasoningEffort: "high"` (with `build.sameAsChat: true` while `AI_BUILD_MODEL` is unset), and `image` on `gemini-3.1-flash-lite-image` with `pinnedToLite: false`. There is no `misrouted` or `refuseReason` field any more — the route guard was removed, so nothing is judged before it is tried. A session that cannot reach the deployment can run `npx vitest run convex/routing.test.ts convex/buildFlow.test.ts` instead, which drives the same settings through the real build pipeline.
- **When the route looks right and builds still fail:** `npx convex run probe:chat` makes one small call to the configured chat route and reports what came back — HTTP status, which fields the reply carried, how many characters went to `content` and how many to a reasoning field, `finish_reason`, and the provider's own error. It describes the key (length, stray whitespace from a paste, masked ends) but never returns it, and scrubs the key out of anything the provider echoes. `generate:routing` says where a turn is sent; this says what answers. Gemini 3.8 Flash thinking is billed inside the output cap (`max_tokens`). High effort uses more of that budget. The model's documented max output is 65536; if `AI_MAX_TOKENS` is still the DeepSeek 393216, the provider may 400 and `complete` will retry inside the named cap. Prefer `AI_MAX_TOKENS=65536` or unset. Empty `content` beside a large `reasoningChars` means the budget went on thinking; retrying buys the same answer.
