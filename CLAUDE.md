...
- **Design (SkillUI Ultra and crews):** research runs in the design worker (`design-worker/`): Brave finds a reference, the page discovery agent chooses its pages -- five at most -- and `skillui --url <reference> --mode ultra --screens 5 --format both` extracts its design. The `.skill` package is stored per site with its extract and its tokens as a foundation stylesheet (`convex/siteDesign.ts`). Every first build and rebuild is written a page at a time by a crew of builders (`convex/crew.ts`, `convex/buildDraft.ts`): one for the header, two for the body, one for the footer, each working to the extract. A page is kept once every part has come back whole and clean; a part that stops the build calls off the replies still being written for the others. The design auditors that checked each part against the extract, and that stopped a build whose part they still sent back after three rounds, were removed on 2026-09-24 at the owner's word: a site is kept as its builders wrote it. The crews run on the page-at-a-time checkpoint steps as they were: each step is an action of its own on a 400s clock (`PAGE_STEP_MS`), a builder the clock stops while writing is saved and carried on from that character in the next step (`draft_partial`), three steps in a row that save nothing stop the build, and so does a part carried on more than four times or a draft past its step ceiling. SkillUI runs untimed in the worker and stops only when Convex closes its request. Edits made in the thread save as they were written; `convex/designGate.ts` is now only the landing step that makes a site's pictures and saves it, in an action of its own. `npx convex run buildDraft:inspect` shows each page's crew and `npx convex run designGate:inspect` recent landings, with the old audit verdicts on rows from before. The retired Awwwards reviewer (`convex/designReview.ts`) stays off.
