...
- **Verify without secrets:** `npx convex run generate:routing` on polished-ram-883. It returns `chat` on `deepseek-flash` at host `api.deepseek.com` (`maxTokens` 32000, `reasoningEffort: "high"`), `strategy` on `deepseek-flash` at the same host with `reasoningEffort: "max"`, `build` on `deepseek-flash` at the same host with `reasoningEffort: "max"`, `maxTokens: 96000` and `sameAsChat: true`, `review` (the retired design reviewer's route, which nothing asks for any more) with `on: false`, and `image` on `gemini-3.1-flash-lite-image` with `pinnedToLite: false`. There is no `misrouted` or `refuseReason` field any more — the route guard was removed, so nothing is judged before it is tried. A session that cannot reach the deployment can run `npx vitest run convex/routing.test.ts convex/buildFlow.test.ts` instead, which drives the same settings through the real build pipeline.
...
- **When a build stops part way:** replies are streamed, and nothing stops one for being slow. A stall is only what the stream itself shows — the connection dropping (`stream_dropped`), the provider erroring part way (`provider_stream_error`), or the model repeating the same passage of its thinking (`looping`). A page that had begun is carried on from where it stopped; a reply that stopped before its page began gets one fresh go. The only clock left is the action's own ten minutes: a reply still going when the words' share runs out (480s, the rest kept for pictures and the save) is recorded as `out_of_time` with the phase it was in, never as a stall. `npx convex run diagnostics:inspectStalls` lists recent stops with where each reply had got to, and `npx convex run probe:stream` makes one small streamed call through the same reader. `npx convex run diagnostics:trace` is the debugger for one build (the latest that failed, or `'{"runId":"..."}'` for any): the line that stopped it and what went wrong just before, each agent's calls with what they produced and why any reply could not be used, and every line of the log with its timing -- the same report support is emailed, with no email set up. A run that has ended takes no more lines, and once one part stops a build the replies still being written for the others are called off, so a failed build's log ends where it stopped; its Build activity marks the lines that say why. `AI_STREAM=0` turns streaming off for a provider that cannot stream. Every build that fails, and every build a reply stopped part way through, is emailed with its whole log to support@forgenexxus.com through Resend the moment it ends (`convex/support.ts`; `SUPPORT_EMAIL` and `SUPPORT_EMAIL_FROM` change the address and sender, `SUPPORT_REPORTS=0` stops them), and the run's log records whether the email went. `npx convex run support:test` sends one test report and says what Resend answered. **Paused on polished-ram-883** (`SUPPORT_REPORTS=0`) until forgenexxus.com is verified in Resend: Resend refuses the shared `onboarding@resend.dev` sender for any address but the account owner's, and refuses `reports@forgenexxus.com` until the domain is verified. `SUPPORT_EMAIL_FROM` is already `Forge Nexxus <reports@forgenexxus.com>`, so once the domain is verified, turning reports on is `npx convex env remove SUPPORT_REPORTS` and then `npx convex run support:test` to confirm one arrives. Sign-in links share that sender (`AUTH_EMAIL_FROM` is unset), so email sign-in only reaches the Resend account owner until the same fix is made for them.
- **Design (crews):** every first build and rebuild is written a page at a time by a crew of builders (`convex/crew.ts`, `convex/buildDraft.ts`): one for the header, two for the body, one for the footer, each working from the brief to the house design rules and told what each page is for. The pages come from the member's answer to *What should people be able to do on your site?* (`FEATURE_PAGES` in `convex/onboardingQuestions.ts`, `sitePages` in `convex/pages.ts`): the home page first, five at most. A page is kept once every part has come back whole and clean; a part that stops the build calls off the replies still being written for the others. The design worker (`design-worker/`: Brave search, the page discovery agent and SkillUI Ultra, on the Mango VM) was deleted on 2026-09-24, and with it `DESIGN_WORKER_URL` and `DESIGN_WORKER_TOKEN`, which nothing reads: a build waits on nothing but the model and the pictures. `siteDesignPackages` stays in the schema only so the rows it saved still validate; nothing reads them, and a site's row goes when the site is rebuilt or deleted. The design auditors that checked each part against the extract, and that stopped a build whose part they still sent back after three rounds, were removed on 2026-09-24 at the owner's word: a site is kept as its builders wrote it. The crews run on the page-at-a-time checkpoint steps as they were: each step is an action of its own on a 400s clock (`PAGE_STEP_MS`), a builder the clock stops while writing is saved and carried on from that character in the next step (`draft_partial`), three steps in a row that save nothing stop the build, and so does a part carried on more than four times or a draft past its step ceiling. Edits made in the thread save as they were written; `convex/designGate.ts` is now only the landing step that makes a site's pictures and saves it, in an action of its own. `npx convex run buildDraft:inspect` shows each page's crew and `npx convex run designGate:inspect` recent landings, with the old audit verdicts on rows from before. The retired Awwwards reviewer (`convex/designReview.ts`) stays off.
