...
- **Design review (the second agent):** a build's header, dropdown menu and footer are held until a separate reviewer agrees they match the Awwwards originals the design agent named in its `clones` block (`convex/designCheck.ts`, `convex/designReview.ts`). The reviewer rides the build route with its own instructions and never sees the design agent's. Disagreement sends the shell back with the reviewer's fixes. After `DESIGN_REVIEW_ROUNDS` reworks (3 unless set) the build is not saved and its credits go back. Every first build is checked; an edit only when its shell changed. `DESIGN_REVIEW=0` turns it off. `npx convex run designReview:inspect` shows recent verdicts and fixes. Tests default it off in `vitest.config.mts`; `convex/designReview.test.ts` turns it on.
