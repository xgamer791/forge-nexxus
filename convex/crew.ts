...
// top of the page and the rest of it -- and one for the footer. Each works
// from the brief to the house design rules every build turn carries, knowing
// what each page of the site is for, and a page is kept once every one of its
// parts has come back whole and clean.
//
...
import { pagePurpose } from "./onboardingQuestions";
...
// Forge's floor under every part: the box model, a body that never scrolls
// sideways, pictures that fit their column, and the type variables the
// header's builder sets from Fontshare.
...
function foundationNote() {
  return [
    ...
    "The foundation sets the box model and the type variables. Set type with var(--font-display) and var(--font-body).",
    "```css",
    FOUNDATION,
    "```",
  ].join("\n");
...
// What each page of the site is for, where the plan says (onboardingQuestions.ts).
function purposes(routes: string[]) {
  const known = routes.flatMap((route) => {
    const purpose = pagePurpose(route);
    return purpose ? [`- ${route}: ${purpose}`] : [];
  });
  return known.length ? [`What each page is for:\n${known.join("\n")}`] : [];
}
...
function crewOrder(routes: string[], path: string) {
  return [
    ...
    ...purposes(routes),
    "Each page has a crew: a builder for the header, two builders for the body -- the top of the page and the rest of it -- and a builder for the footer. Build every part from the brief to the design rules above, and keep every page of the site to one design.",
...
const SECTION_RULES = [
  "- Build the sections this page needs for what it is for, at phone and desktop widths. A page after the home page keeps to the site so far: its type, colour, spacing and components.",
  "- Write every word from the brief, and ask for every picture through forge-image.",
].join("\n");

function headerAsk(routes: string[]) {
  return [
    "You are the header builder. Write the site's header: the brand, the navigation, and on a phone a menu that opens and closes.",
...
function footerAsk(routes: string[]) {
  return [
    "You are the footer builder. Write the site's footer, at phone and desktop widths.",
...
// One builder's turn: the rules every build turn carries, the foundation and
// the crew, the brief, and then its own part to write.
export function builderTurn(input: BuilderInput): Message[] {
  const { part, path } = input;
  const messages: Message[] = [
    ...input.base,
    { role: "system", content: foundationNote() },
...
export function shellFrom(input: { header: string; footer: string; siteName: string }) {
