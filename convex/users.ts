...
export async function purgeUser(ctx: MutationCtx, userId: Id<"users">) {
  ...
  const owned = [
    ...
    ...(await ctx.db.query("buildRuns").withIndex("by_user", (q) => q.eq("userId", userId)).collect()),
    ...
    ...(await ctx.db.query("pageAgents").withIndex("by_user", (q) => q.eq("userId", userId)).collect()),
