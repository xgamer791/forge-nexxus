...
import { gateInFlight } from "./designGate";
...
function buildMessages(
  ...
  if (shown) {
    messages.push({
      role: "system",
      content: `The site "${siteName}" currently looks like this. Apply the user's next request to it and return the whole updated site, every block, in the same form. Design from the saved brief and from DESIGN_GOD, and write original copy.\n\n${shown}`,
    });
  }
  if (talkOnly) {
    messages.push({
