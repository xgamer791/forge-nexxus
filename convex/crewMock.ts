// Test double for the crew's builders: how a builder's call is recognised, and
// what a well-behaved builder writes back. CI never calls a live model.
...
function partMarkup(call: Pick<CrewCall, "part" | "path">, label = "first") {
  ...
  switch (call.part) {
    case "header":
      return `<link rel="stylesheet" href="https://api.fontshare.com/v2/css?f[]=switzer@400,600&display=swap">` +
        ...
        `<style>:root{--font-display:'Switzer',system-ui,sans-serif}.site-header{display:flex;padding:16px}</style>` +
        `<header class="site-header"><a href="/" class="hd-brand">Harbor Roasters</a><nav><a href="/">Home</a></nav></header>`;
    case "footer":
      return `<style>.site-footer{padding:16px}</style><footer class="site-footer"><p>Harbor Roasters, Port Ellen pier</p></footer>`;
    case "body1":
      return `<style>.a-open{min-height:80svh}</style><section class="a-open"><h1>${label} ${slug}</h1>${PART_PICTURE}</section>`;
    case "body2":
      return `<style>.b-more{padding:16px}</style><section class="b-more"><h2>More about ${slug}</h2><p>The ${label} ${slug} page.</p></section>`;
  }
}
