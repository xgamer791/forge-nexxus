...
html = html.replace(/(\.\/(?:styles\.css|app\.js|forge-data\.js)\?v=)[^"']+/g, '$1development');
const assets = await Promise.all(['app.js', 'styles.css', 'forge-data.js'].map(name => readFile(new URL(`../docs/${name}`, import.meta.url), 'utf8')));
...
html = html.replace(/(\.\/(?:styles\.css|app\.js|forge-data\.js)\?v=)[^"']+/g, `$1${version}`);
