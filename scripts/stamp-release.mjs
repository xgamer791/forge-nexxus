...
html = html.replace(/(\.\/(?:styles\.css|app\.js|forge-data\.js|auth\.css|auth\.js)\?v=)[^"']+/g, '$1development');
const assets = await Promise.all(['app.js', 'styles.css', 'forge-data.js', 'auth.js', 'auth.css'].map(name => readFile(new URL(`../docs/${name}`, import.meta.url), 'utf8')));
...
html = html.replace(/(\.\/(?:styles\.css|app\.js|forge-data\.js|auth\.css|auth\.js)\?v=)[^"']+/g, `$1${version}`);
