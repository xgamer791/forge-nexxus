...
describe("the brief collects what a shop needs", () => {
  test("the last question asks for the catalogue without limiting generated pricing", async () => {
    ...
    expect(QUESTIONS[FINAL_STEP].id).toBe("references");
...
describe("the contract sets a floor, not a mould", () => {
  ...
  test("the reply designs from the brief and DESIGN_GOD instead of naming Awwwards clones", () => {
    expect(contract).toContain("Design from the saved brief and from DESIGN_GOD");
